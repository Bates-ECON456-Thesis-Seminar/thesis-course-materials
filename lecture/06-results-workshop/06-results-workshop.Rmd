...
- Someone should be able to skim your figures and the associated notes and get the gist of the paper
...
- Do not present extensive details on something you are not going to talk about in your paper
    - Oh you have a vector of controls tossed in to your regression to satisfy a conditional independence assumption? Great! 
    - But don't show me every coefficient and standard error for each control variable in a main table
    - Instead mention the controls in the caption and whether they are or are not included in a column
...
- Balance tables are basically just summary statistic tables, but for treatment and control groups

- Generally, you're doing a twoway t-test to compare the two groups, so you'll just present means instead of every statee
...
- First specification should be the main or simplest specification, then each column should be a change from that
    - The changes may be cumulative: simplest, then you add controls, then you add new fixed effects
    - The changes may be distinct by column: simplest, then you add controls, then you remove controls and add fixed effects
    - It depends on the table

- Your specification change may just mean a new dependent variable, in which case put the dependent variable in the header

- Each row should be a variable in the regression or summarize information about the regression ($N$ is number of observations)

- Columns should be numbered

- After variables, throw in a horizontal line and then some summary information:
...
```{r reg,echo=FALSE,warning=FALSE,message=FALSE,out.height='80%'}
data('airquality')
...
#> NOTE: 42 observations removed because of NA values (LHS: 37, RHS: 7).
est_c3 = feols(Ozone ~ Solar.R + Solar.R^2 + ..ctrl | Day, data = airquality)
#> NOTE: 42 observations removed because of NA values (LHS: 37, RHS: 7).
etable(est_c0, est_c1, est_c2, est_c3, group = list(Controls = "poly"), style.df = style.df("aer"),digits=2, postprocess.df = pandoc.table.return, style = "rmarkdown",dict=dict) %>% kable()
```

---
# Regression table example

```{r reg-table-subs,echo=FALSE,warning=FALSE,message=FALSE,out.height='80%'}
...
etable(est_all, est_sub1, est_sub2, est_sub3,
       extralines = list("Sub-sample" = c("All", "May-June", "Jul.-Aug.", "Sept.")), style.df = style.df("aer"), cluster =~Day,markdown=TRUE,dict=dict) %>% kable()
```
...
<center>
<img src="imgs/Yagan2015.jpg" alt="Yagan (2015) shows how C- and S-corporation business practices move togethewr until a dividend tax cut for C-corps, led to more payouts to shareholders."> 
</center>
...
<center>
<img src="imgs/earnin_cps_dist.pdf" alt="Coombs et al. (2021) compares our sample of Earnin users to the Current Population Survey to show a poorer population">
</center
...
- Don't make elaborate 3D figures unless you must
...
- Avoid y-axes that don't start at 0
    - This can be misleading, but you can break this rule

- Avoid having a separate left and right y axis
    - This can be misleading, but you can break this rule for the right case 
...
<center>
<img src="imgs/diffindiff-gruber.jpg" alt="Gruber diff-in-diff results.">
</center>
...
<center>
<img src="imgs/triplediff-gruber.jpg" alt="Gruber diff-in-diff results.">
</center>
